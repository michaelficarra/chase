	...
	\label{sec:chase.algorithm}
		...
		\While{$\exists$ a formula $\sigma \in T$ and an environment $\lambda$ such that $\mathbb{M} \not\models_\lambda \sigma$}{
			...
			\lIf{$\beta$ contains a disjunction}{
				choose a disjunct and assign it to $\beta$ \;
			}
			\ForEach{variable $v \in free(\beta)$ for which $\lambda$ is undefined}{
				add a new domain member $\omega$ to $|\mathbb{M}|$ such that $\omega \not\in |\mathbb{M}|$ \;
				redefine $\lambda$ as $\lambda_{v \mapsto \omega}$ \;
			}
			\ForEach{relation $R(\alpha_0,\ldots,\alpha_n) \in \mathbb{M}$}{
