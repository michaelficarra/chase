...
%		[yours]
%
%			When $\sigma$ is in the form $E \to \bigvee_i \bigwedge_j R_{ij}(\vec
%			x)$, we know that $\mathbb{M} \models_\lambda \sigma$ because
%			$\mathbb{M} \models T$. Therefore, $\mathbb{M} \models \bigwedge_j
%			R_{ij}(\vec x)$ for some disjunct $i$. There exists a chase step
%			$\mathbb{N} \xrightarrow{(\sigma,\lambda)} \mathbb{N}'$ that will
%			choose this disjunct. This chase step proceeds normally and generates
%			$\mathbb{N}'$. The homomorphism $h : \mathbb{N} \to \mathbb{M}$ is also
%			a homomorphism from $\mathbb{N}'$ to $\mathbb{M}$ because, for every
%			$R_{\mathbb{N}'}(a_0,\ldots,a_n)$, there exists
%			$R_\mathbb{M}(h(a_0),\ldots,h(a_n))$.
%
%		[my first]
%
%			Suppose $\sigma$ is in the form $E(\vec{x}) \to \bigvee_i
%			\bigwedge_j R_{ij}(\vec x)$. Since $\sigma$ is false in
%			$\mathbb{N}$ under $\lambda$, we have $\mathbb{N} \models
%			E(\vec{x})$ under $\lambda$ while $\mathbb{N} \not\models
%			\bigvee_i \bigwedge_j R_{ij}(\vec x)$ under $\lambda$. Since $h$
%			is a homomorphism, $\mathbb{M} \models E(\vec{x})$ under $h \circ
%			\lambda$. Since $M \models \sigma$ under $h \circ \lambda$, we
%			have $\mathbb{M} \models \bigvee_i \bigwedge_j R_{ij}(\vec x)$
%			under $h \circ \lambda$. Therefore, $\mathbb{M} \models
%			\bigwedge_j R_{ij}(\vec x)$ under $h \circ \lambda$ for some
%			disjunct $i$. There exists a chase step $\mathbb{N}
%			\xrightarrow{(\sigma,\lambda)} \mathbb{N}'$ that will choose this
%			disjunct. This chase step proceeds normally and generates
%			$\mathbb{N}'$, by adding the facts $R_{ij}(\lambda(\vec{x}))$ for
%			each $j$. To see that $h : \mathbb{N} \to \mathbb{M}$ is also a
%			homomorphism from $\mathbb{N}'$ to $\mathbb{M}$ it suffices to see
%			that each of the new facts added to $\mathbb{N}'$ is preserved, by
%			$h$, in $\mathbb{M}$. That is, we want to see that for each $j$,
%			the fact $R_{ij}(h(\lambda(x_0)),\ldots,h(\lambda(x_n))$ holds in
%			$\mathbb{M}$. But this follows from our earlier observation that
%			$\mathbb{M} \models \bigwedge_j R_{ij}(\vec x)$ under $h \circ
%			\lambda$.
%
%		[real thing]
