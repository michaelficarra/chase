...
\label{sec:implementation}
	...
	\ref{fairness_definition}. Essentially, an implementation is fair if
	\textbf{need help writing this}.

	Though the discussed implementation is quite efficient, it is not fair. The
	...
	\subsection{Operation}
		...
		formula given. {\tt satisfy} will behave as outlined in the following
		algorithm.
		...
		\begin{algorithm}[H]
		\DontPrintSemicolon
		\TitleOfAlgo{satisfy :: Model ($\mathbb{M}$) $\to$ Environment ($\lambda$) $\to$ Sentence ($\varphi$) $\to$ [Model]}
		return \Switch{$\varphi$}{
			\lCase{$\top$}{return $\{ \mathbb{M} \}$} \;
			\lCase{$\bot$}{return $\emptyset$} \;
			\Case{$x=y$}{
				let $a=\lambda(x)$ and $b=\lambda(y)$
				\lIf{$a=b$}{return $\mathbb{M}$} \;
				let $\mathbb{N}$ be a model where $|\mathbb{N}| = |\mathbb{M}|$ \;
				\ForEach{fact $R_\mathbb{M}(\vec v)$}{
					let $\vec \eta = \vec v$ with all references to $b$ replaced with $a$ \;
					add $R(\vec \eta)$ to $\mathbb{N}$ \;
				}
				return $\mathbb{N}$
			}
			...
			\Case{$\alpha \to \beta$}{
				\lIf{$\mathbb{M} \models_\lambda \alpha$}{return $satisfy(\mathbb{M},\lambda,\beta)$ \;}
		...
		\textbf{should probably move this to an appendix}
		\textbf{also, need some rewording help from dd at least for the x=y section}
