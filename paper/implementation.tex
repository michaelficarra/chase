...
\label{sec:implementation}
	...
	return \Switch{$\varphi$}{
		...
		\Case{$\exists\ \vec x : \alpha$}{
			\lIf{$\vec x = \emptyset$}{ $satisfy(\alpha)$ } \;
	...
	\subsection{Input Format}
		...
		Input to the parser must be in a form parsable by the context-free
		grammar seen in Appendix \ref{sec:appendixParser}. Terminals are
		...
		in the table in Appendix \ref{sec:appendixLexer}.
