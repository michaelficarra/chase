...
\label{sec:technical_background}
	...
	\subsection{Positive Existential Form}
		...
		\begin{proof}
			It suffices to prove the claim about positive-existential sentences
			by proving the claim for positive-existential \emph{formul{\ae}} by
			induction over formul{\ae}. Given $\sigma$ is a
			positive-existential formula, $\mathbb{M}$ is a model, $\lambda :
			[var] \to |\mathbb{M}|$ is an environment such that $\mathbb{M}
			\models_\lambda \sigma$, and $h$ is a homomorphism $h :
			|\mathbb{M}| \to |\mathbb{M}'|$, then we want to show that
			$\mathbb{M}' \models_{h\circ\lambda} \sigma$.

			When $\sigma$ is an atomic formula $R(x_0,\ldots,x_n)$, we know
			that $R(\lambda(x_0),\ldots,\lambda(x_n)) \in \mathbb{M}$ and by the
			definition of a homomophism,
			$R((h\circ\lambda)(x_0),\ldots,(h\circ\lambda)(x_n)) \in
			\mathbb{M}'$.
			...
			When $\sigma$ is $\alpha \vee \beta$, 

			When $\sigma$ is $\exists\ \vec x : \alpha$, we know
			that $\mathbb{M} \models_\lambda \exists\ x : \alpha$, and want to
			prove that $\mathbb{M}' \models_{h\circ\lambda} \exists\ x :
			\alpha$. There is a $\delta \in |\mathbb{M}|$ such that $\mathbb{M}
			\models_{\lambda[x\mapsto\delta]} \alpha$.

			\textbf{we say something else and finally get to...}

			$\mathbb{M}' \models_{(h\circ\lambda)[x\mapsto h(\delta)]} \alpha$
	...
	\label{sec:technical_background.geometric_logic}
		...
		R'$, and using $R'$ where $\neg R$ would be used. Constant symbols can
		be encoded as relations with an arity of zero, for example $C()$. A
		function $f(x,y)$ can be encoded as the geometric logic formula
