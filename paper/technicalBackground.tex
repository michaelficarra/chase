...
\label{sec:technical_background}
	...
	\label{sec:technical_background.minimal_models}

		\emph{Minimal models}, also called \emph{universal} models, are models
		for a theory $T$ with the special property that there exists a
		homomorphism from the minimal model to any other model that satisfies
		$T$. Intuitively, minimal models have no unnecessary entities or
		relations and thus display the least amount of constraint necessary to
		satisfy the theory for which they are minimal. Any model to which there
		exists a homomorphism from a minimal model will have more constraints
		than the minimal model.

		A set of models $\mathcal{M}$ is said to be \emph{jointly minimal} for
		a set of formul{\ae} $\Sigma$ when, for every model $\mathbb{N}$ such that
		$\mathbb{N} \models \Sigma$, there exists a homomorphism from a model $\mathbb{M}
		...
		set of fact would be jointly minimal.
