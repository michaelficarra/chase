...
\label{sec:technical_background}
	...
	\subsection{Positive Existential Form}
		...
		\begin{proof}
			Suppose a positive-existential formula $\sigma$.

			When $\sigma$ is an atomic formula $R(x_0,\ldots,x_n)$, 
			...
			When $\sigma$ is $\alpha \wedge \beta$, 

			When $\sigma$ is $\alpha \vee \beta$, 

			When $\sigma$ is $\exists\ \vec x : \alpha$, 
		\end{proof}
